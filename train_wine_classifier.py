#!/usr/bin/env python3
"""
Train a CNN model for wine adulteration classification (per-frame basis).
...
if '--help' in sys.argv or '-h' in sys.argv:
    print("""usage: train_wine_classifier.py [-h] [--batch-size BATCH_SIZE]
                                          [--seed SEED] [--n-regions N_REGIONS]
                                          [--classes CLASSES]
                                          [--binary] [--year-classification]

Train wine adulteration classifier with CNN (per-frame)
...
optional arguments:
  ...
  --classes CLASSES     Comma-separated list of adulteration classes to use.
                        Available: unadulterated, coke, ethanol, tylenol, vinegar, water, asa
                        Can be used with --binary to select which classes to include
                        (default: all classes)
  --binary              Binary classification: adulterated vs unadulterated
                        (default: False - multi-class classification)
                        Can be combined with --classes to select specific types
  --year-classification
                        Binary classification: 2020 wine vs 2023 wine
                        (ignores adulteration types)
...
if '--info' in sys.argv:
    print("""
╔══════════════════════════════════════════════════════════════════════╗
║     WINE ADULTERATION CLASSIFIER TRAINING (PER-FRAME)                ║
╚══════════════════════════════════════════════════════════════════════╝

This script trains a CNN model to classify wine adulteration from
...
- session_wine_2020_regions.npz (unadulterated)
- session_wine_2023_regions.npz (unadulterated)
- session_wine_2023_coke_regions.npz
- session_wine_2023_ethanol_regions.npz
- session_wine_2023_tylenol_regions.npz
- session_wine_2023_vinegar_regions.npz
- session_wine_2023_water_regions.npz
- session_wine_2023_asa_regions.npz

Multiple files per type are supported with numbered suffixes:
- session_wine_2020_2_regions.npz, session_wine_2020_3_regions.npz, etc.
- session_wine_2023_coke_2_regions.npz, session_wine_2023_coke_3_regions.npz, etc.
...
1. Multi-class classification (default):
   ...
   - Can select specific classes to include with --classes

2. Binary classification (--binary):
   - Classify as adulterated vs unadulterated
   - All adulteration types grouped into one class
   - Can be combined with --classes to select specific types to include

3. Year classification (--year-classification):
   - Binary classification: 2020 wine vs 2023 wine
   - Ignores adulteration types
   - Uses only unadulterated wine from both years
...
python3 train_wine_classifier.py

# Train binary classification (adulterated vs unadulterated)
python3 train_wine_classifier.py --binary

# Train binary classification with only specific classes
python3 train_wine_classifier.py --binary --classes unadulterated,coke,water

# Train only on specific classes (multi-class)
python3 train_wine_classifier.py --classes unadulterated,coke,water

# Train with different number of regions
python3 train_wine_classifier.py --n-regions 4

# Train with all combinations (may have data leakage)
python3 train_wine_classifier.py --n-regions 2 --use-all-combinations

# Train year classification (2020 vs 2023 wine)
python3 train_wine_classifier.py --year-classification
          
# Sequence length
python3 train_wine_classifier.py --sequence-length 50,20,10,5,2,1 --epochs 10 --classes unadulterated,ethanol20,ethanol30,ethanol40,ethanol60,ethanol80 --binary
...
ALL_ADULTERATIONS = ['unadulterated', 'coke', 'ethanol', 'tylenol', 'vinegar', 'water', 'asa', 'ethanol10', 'ethanol20', 'ethanol30', 'water30', 'ethanol40', 'ethanol60', 'ethanol80']

def get_binary_label(adulteration):
    """Convert adulteration type to binary label for unadulterated vs adulterated classification."""
...
class AdulterationDataset(Dataset):
    """Dataset for wine adulteration classification from temporal patches."""
...
class SpatioTemporalCNN(nn.Module):
    ...
    def __init__(self, input_size=7, num_classes=7, hidden_dim=64, lstm_layers=2):
...
def load_wine_adulteration_data(patches_dir="regions", sequence_length=50, n_regions=6, 
                                use_all_combinations=False, selected_classes=None, binary=False,
                                year_classification=False):
    """
    Load wine adulteration patch data and create samples by batching regions.
    ...
    year_classification : bool
        If True, binary classification (2020 wine vs 2023 wine).
        Ignores adulteration types.
    ...
    # Year classification mode: only load unadulterated wine from 2020 and 2023
    if year_classification:
        print("Year classification mode: 2020 wine vs 2023 wine")
        adulteration_data = defaultdict(list)
        original_lengths = []
        
        # Load 2020 wine (including numbered variants)
        files_2020 = []
        file_2020 = patches_dir / 'session_wine_2020_regions.npz'
        if file_2020.exists():
            files_2020.append(file_2020)
        # Check for numbered variants: session_wine_2020_2_regions.npz, session_wine_2020_3_regions.npz, etc.
        for i in range(2, 100):  # Check up to 100 variants
            file_variant = patches_dir / f'session_wine_2020_{i}_regions.npz'
            if file_variant.exists():
                files_2020.append(file_variant)
            elif i > 10 and not file_variant.exists():
                break  # Stop searching after 10 consecutive missing files
        
        if not files_2020:
            raise ValueError("2020 wine file not found: session_wine_2020_regions.npz")
        
        print(f"  Found {len(files_2020)} file(s) for 2020 wine")
        for file in files_2020:
            ...
            for patch in region_patches:
                ...
                adulteration_data['wine_2020'].append(patch)
        
        # Load 2023 wine (unadulterated only, including numbered variants)
        files_2023 = []
        file_2023 = patches_dir / 'session_wine_2023_regions.npz'
        if file_2023.exists():
            files_2023.append(file_2023)
        # Check for numbered variants: session_wine_2023_2_regions.npz, session_wine_2023_3_regions.npz, etc.
        for i in range(2, 100):  # Check up to 100 variants
            file_variant = patches_dir / f'session_wine_2023_{i}_regions.npz'
            if file_variant.exists():
                files_2023.append(file_variant)
            elif i > 10 and not file_variant.exists():
                break  # Stop searching after 10 consecutive missing files
        
        if not files_2023:
            raise ValueError("2023 wine file not found: session_wine_2023_regions.npz")
        
        print(f"  Found {len(files_2023)} file(s) for 2023 wine")
        for file in files_2023:
            ...
            for patch in region_patches:
                ...
                if len(patch) < sequence_length:
                    ...
                    patch = patch[:sequence_length]
                adulteration_data['wine_2023'].append(patch)
        ...
        all_spatial_dims = []
        for year, patches in adulteration_data.items():
        ...
        for year in adulteration_data.keys():
            resized_patches = []
            for patch in adulteration_data[year]:
            ...
            adulteration_data[year] = resized_patches
        
        # Create labels for year classification
        adulteration_names = ['wine_2020', 'wine_2023']
        label_to_adulteration = {0: 'wine_2020', 1: 'wine_2023'}
        adulteration_to_label = {'wine_2020': 0, 'wine_2023': 1}
        
    # Regular adulteration classification modes
    else:
        # Determine which classes to load
        if selected_classes is None:
            # No classes specified, use all available
        ...
        for adulteration in adulterations_to_load:
            # Wine has different file naming patterns
            # Unadulterated: session_wine_2020_regions.npz and session_wine_2023_regions.npz
            # Adulterated: session_wine_2023_{adulteration}_regions.npz
            
            matching_files = []
            
            if adulteration == 'unadulterated':
                # Check for both 2020 and 2023 unadulterated files (including numbered variants)
                # 2020 files
                file_2020 = patches_dir / 'session_wine_2020_regions.npz'
                if file_2020.exists():
                    matching_files.append(file_2020)
                # Check for numbered variants: session_wine_2020_2_regions.npz, etc.
                for i in range(2, 100):
                    file_variant = patches_dir / f'session_wine_2020_{i}_regions.npz'
                    if file_variant.exists():
                        matching_files.append(file_variant)
                    elif i > 10 and not file_variant.exists():
                        break
                ...
                file_2023 = patches_dir / 'session_wine_2023_regions.npz'
                if file_2023.exists():
                    matching_files.append(file_2023)
                # Check for numbered variants: session_wine_2023_2_regions.npz, etc.
                for i in range(2, 100):
                    file_variant = patches_dir / f'session_wine_2023_{i}_regions.npz'
                    if file_variant.exists():
                        matching_files.append(file_variant)
                ...
                # Adulterated wine: session_wine_2023_{adulteration}_regions.npz (including numbered variants)
                file_path = patches_dir / f'session_wine_2023_{adulteration}_regions.npz'
                if file_path.exists():
                    matching_files.append(file_path)
                # Check for numbered variants: session_wine_2023_{adulteration}_2_regions.npz, etc.
                for i in range(2, 100):
                    file_variant = patches_dir / f'session_wine_2023_{adulteration}_{i}_regions.npz'
                    if file_variant.exists():
                        matching_files.append(file_variant)
                    elif i > 10 and not file_variant.exists():
                        break
        ...
        if binary:
            ...
            spiking_to_label = {s: get_binary_label(s) for s in adulteration_data.keys()}
        else:
    ...
    for adulteration, patches in adulteration_data.items():
        ...
        if year_classification:
            ...
            label = spiking_to_label[adulteration]
        else:
            label = adulteration_to_label[adulteration]
...
def plot_results(results, save_dir='results/wine_adulteration', run_name=None):
    """Plot training curves, confusion matrix, and confidence scores for cross-validation."""
    os.makedirs(save_dir, exist_ok=True)
    
    if run_name is None:
        run_name = f"wine_adulteration"
    ...
    ax6.axis('off')
    summary_text = f"""
    Wine Adulteration Classification Results
    ...
    with open(f'{save_dir}/cv_summary_{run_name}.txt', 'w') as f:
        f.write(f"Wine Adulteration Classification Results\n")
    ...
    sequence_length = results.get('sequence_length', 50)  # default to 50 for wine
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train wine adulteration classifier (per-frame)')
    ...
    parser.add_argument('--classes', type=str, default=None,
                       help='Comma-separated list of adulteration classes to use. Can be combined with --binary (default: all)')
    parser.add_argument('--binary', action='store_true',
                       help='Binary classification: adulterated vs unadulterated. Can be combined with --classes to select specific types (default: False)')
    parser.add_argument('--year-classification', action='store_true',
                       help='Binary classification: 2020 wine vs 2023 wine (default: False)')
    ...
    if args.binary and args.year_classification:
        print("Error: Cannot use both --binary and --year-classification modes")
        return
    
    # Parse selected classes
    if args.classes is not None:
        if args.year_classification:
            print("Warning: --classes argument ignored in year classification mode")
            selected_classes = None
        else:
            selected_classes = [c.strip() for c in args.classes.split(',')]
            print(ALL_ADULTERATIONS)
            # Validate classes
            ...
            if args.binary:
                ...
                print("Classes will be grouped into adulterated vs unadulterated")
    else:
        selected_classes = None
    ...
    for seq_idx, sequence_length in enumerate(sequence_lengths):
        ...
        print("Loading wine adulteration data...")
        ...
        data = load_wine_adulteration_data(n_regions=args.n_regions,
                                           sequence_length=sequence_length, 
                                           use_all_combinations=args.use_all_combinations,
                                           selected_classes=selected_classes,
                                           binary=args.binary,
                                           year_classification=args.year_classification)
        ...
        run_name = f"wine_{class_mode}_n{args.n_regions}_{combo_mode}{seq_len_str}"
