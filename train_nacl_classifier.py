#!/usr/bin/env python3
"""
Train a CNN model for NaCl concentration classification (per-frame basis).
...
if '--help' in sys.argv or '-h' in sys.argv:
    print("""usage: train_nacl_classifier.py [-h] [--batch-size BATCH_SIZE]
                                          [--seed SEED] [--n-regions N_REGIONS]
                                          [--classes CLASSES]

Train NaCl concentration classifier with CNN (per-frame)
...
optional arguments:
  ...
  --classes CLASSES     Comma-separated list of concentration classes to use.
                        Available: 0-00005, 0-0001, 0-001, 0-01, 0-1, etc.
                        (default: all classes)
...
if '--info' in sys.argv:
    print("""
╔══════════════════════════════════════════════════════════════════════╗
║     NACL CONCENTRATION CLASSIFIER TRAINING (PER-FRAME)               ║
╚══════════════════════════════════════════════════════════════════════╝

This script trains a CNN model to classify NaCl concentration from
...
- session_conc_nacl_0-00005_regions.npz (or session_conc_nacl_0-00005_*_regions.npz)
- session_conc_nacl_0-0001_regions.npz (or session_conc_nacl_0-0001_*_regions.npz)
- session_conc_nacl_0-001_regions.npz (or session_conc_nacl_0-001_*_regions.npz)
- session_conc_nacl_0-01_regions.npz (or session_conc_nacl_0-01_*_regions.npz)
- session_conc_nacl_0-1_regions.npz (or session_conc_nacl_0-1_*_regions.npz)
- etc.
...
EXAMPLES
──────────────────────────────────────────────────────────────────────
# Train with all NaCl concentrations (default)
python3 train_nacl_classifier.py

# Train only on specific concentrations
python3 train_nacl_classifier.py --classes 0-00005,0-0001,0-001,0-01,0-1

# Train with different number of regions
python3 train_nacl_classifier.py --n-regions 4

# Train with all combinations (may have data leakage)
python3 train_nacl_classifier.py --n-regions 2 --use-all-combinations
...
# All available NaCl concentrations
ALL_CONCENTRATIONS = ['0', '0-00005', '0-0001', '0-0002', '0-0005', '0-001', '0-002', '0-005', '0-01', '0-02', '0-05', '0-1', '0-25', '0-5', '0-9', '1-0', '2-0', '5-84']
...
class NaClDataset(Dataset):
    """Dataset for NaCl concentration classification from temporal patches."""
...
def load_nacl_data(patches_dir="regions", sequence_length=50, n_regions=1,
                   use_all_combinations=False, selected_classes=None):
    """
    Load NaCl concentration patch data and create samples by batching regions.
    ...
    selected_classes : list
        List of concentration classes to include. If None, use all available.

    Returns:
    --------
    data : dict
        Contains 'combinations', 'combo_labels', 'combo_concentrations',
        'concentration_names', 'label_to_concentration', 'sequence_length'
    """
    patches_dir = Path(patches_dir)

    # Determine which classes to load
    if selected_classes is None:
        concentrations_to_load = ALL_CONCENTRATIONS
        print("Loading all NaCl concentration classes")
    else:
        concentrations_to_load = selected_classes
        print(f"Loading NaCl concentration classes: {concentrations_to_load}")

    # Group by concentration
    concentration_data = defaultdict(list)
    original_lengths = []

    for concentration in concentrations_to_load:
        # Find all files matching this concentration (handles multiple sessions)
        pattern1 = f'session_conc_nacl_{concentration}_regions.npz'  # without number
        pattern2 = f'session_conc_nacl_{concentration}_*_regions.npz'  # with number
        ...
        if not matching_files:
            print(f"Warning: No files found for concentration {concentration}, skipping")
            continue

        print(f"  Found {len(matching_files)} file(s) for {concentration}")
        ...
        for file in matching_files:
            ...
            for patch in region_patches:
                ...
                concentration_data[concentration].append(patch)

    if not concentration_data:
        raise ValueError("No data loaded! Check that the region files exist.")

    print(f"Loaded data for: {sorted(concentration_data.keys())}")
    ...
    all_spatial_dims = []
    for concentration, patches in concentration_data.items():
    ...
    for concentration in concentration_data.keys():
        resized_patches = []
        for patch in concentration_data[concentration]:
        ...
        concentration_data[concentration] = resized_patches

    # Create labels (sort by numeric value for logical ordering)
    def conc_to_float(conc_str):
        """Convert concentration string to float for sorting (e.g., '0-001' -> 0.001)"""
        if conc_str == '0':
            return 0.0
        return float(conc_str.replace('-', '.'))

    concentration_names = sorted(concentration_data.keys(), key=conc_to_float)
    ...
    for concentration, patches in concentration_data.items():
        print(f"\n{concentration}: {len(patches)} regions")
        ...
        label = concentration_to_label[concentration]
        ...
        if use_all_combinations:
            ...
            for combo_indices in all_combos:
                ...
                all_combo_concentrations.append(concentration)
    ...
    for i, name in enumerate(concentration_names):
...
def train_cross_validation(data, n_folds=5, epochs=100, batch_size=16, lr=0.001):
    ...
    combo_concentrations = data['combo_concentrations']
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits):
        ...
        conc_train = combo_concentrations[train_idx]
        conc_test = combo_concentrations[test_idx]
        ...
        X_test = []
        y_test = []
        y_test_concentrations = []
        for combo, label, conc in zip(combo_test, label_test, conc_test):
            for frame_idx in range(combo.shape[0]):
                X_test.append(combo[frame_idx])
                y_test.append(label)
                y_test_concentrations.append(conc)
        ...
        # Create datasets
        train_dataset = NaClDataset(X_train, y_train, concentration_names)
        test_dataset = NaClDataset(X_test, y_test, concentration_names,
        ...
        all_test_concentrations.extend(y_test_concentrations)

    # Compute overall statistics
    all_test_preds = np.array(all_test_preds)
    all_test_true = np.array(all_test_true)
    all_test_probs = np.array(all_test_probs)
    all_test_concentrations = np.array(all_test_concentrations)
    ...
    return {
        ...
        'all_test_concentrations': all_test_concentrations,
        'concentration_names': concentration_names,
        'n_folds': n_folds,
        'sequence_length': sequence_length
    }

def plot_results(results, save_dir='results/nacl', run_name=None):
    """Plot training curves, confusion matrix, and confidence scores for cross-validation."""
    os.makedirs(save_dir, exist_ok=True)

    if run_name is None:
        run_name = f"nacl_classifier"
    ...
    sns.heatmap(cm_percent, annot=annotations, fmt='', cmap='Blues',
                xticklabels=concentration_names, yticklabels=concentration_names, ax=ax4,
    ...
    ax6.axis('off')
    summary_text = f"""
    NaCl Concentration Classification Results
    {n_folds}-Fold Cross-Validation

    Model: CNN (per-frame)
    Classes: {len(concentration_names)}
    ...
    for conc_name in concentration_names:
    ...
    with open(f'{save_dir}/cv_summary_{run_name}.txt', 'w') as f:
        f.write(f"NaCl Concentration Classification Results\n")
        f.write(f"{n_folds}-Fold Cross-Validation\n")
        f.write(f"{'='*60}\n\n")
        f.write(f"Model: CNN (per-frame)\n")
        f.write(f"Classes: {len(concentration_names)}\n\n")
        ...
        f.write(f"{'='*60}\n")
        cm = confusion_matrix(results['all_test_true'], results['all_test_preds'])
        f.write(f"\nActual \\ Predicted: {' '.join([f'{name:>10}' for name in concentration_names])}\n")
        for i, name in enumerate(concentration_names):
            f.write(f"{name:>15}: {' '.join([f'{cm[i][j]:>10}' for j in range(len(concentration_names))])}\n")

    # Save raw predictions for later re-plotting
    predictions_file = f'{save_dir}/cv_predictions_{run_name}.npz'

    # Get sequence_length from fold_results if available
    sequence_length = 30  # default
    if fold_results and 'sequence_length' in results:
    ...
    np.savez(predictions_file,
             ...
             all_test_concentrations=results['all_test_concentrations'],
             concentration_names=results['concentration_names'],
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train NaCl concentration classifier (per-frame)')
    ...
    parser.add_argument('--classes', type=str, default=None,
                       help='Comma-separated list of concentration classes to use (default: all)')
    ...
    if args.classes is not None:
        ...
        # Validate classes
        invalid_classes = [c for c in selected_classes if c not in ALL_CONCENTRATIONS]
        if invalid_classes:
            print(f"Error: Invalid classes: {invalid_classes}")
            print(f"Available classes: {ALL_CONCENTRATIONS}")
            return
    else:
        selected_classes = None

    print("Loading NaCl concentration data...")
    ...
    data = load_nacl_data(n_regions=args.n_regions,
    ...
    if selected_classes is not None:
        ...
        run_name = f"nacl_n{args.n_regions}_{combo_mode}_{class_suffix}"
    else:
        run_name = f"nacl_n{args.n_regions}_{combo_mode}"

    print("\nPlotting results...")
    plot_results(results, run_name=run_name)

    print(f"\nTraining complete! Results saved to results/nacl/")
