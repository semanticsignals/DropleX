...
║                                                                            ║
║ CALIBRATION:                                                              ║
║   Calibrate Btn  - Toggle compensation map on/off                        ║
║                    Loads: calibration/compensation_map_combined_all_      ║
║                           sessions.npy                                    ║
║                    When ON: saves calibrated values in *_calibrated.npz  ║
║   JIT Cal Btn    - Toggle Just-In-Time calibration on/off                ║
║                    Creates compensation map from current frame to reduce  ║
║                    spatial variability within selected regions            ║
║                    Applies to current frame onwards. Evaluates if         ║
║                    compensation improves variability.                     ║
║                    When ON: saves in *_jit_calibrated.npz                ║
║                                                                            ║
...
def flood_fill_region(data, seed_y, seed_x, tolerance):
    ...
    while to_check:
        ...
        if abs(data[y, x] - seed_value) <= tolerance:
            mask[y, x] = True
...
def zscore_threshold(data, z_threshold=2.0, mode='both'):
    ...
    if mode == 'positive':
        ...
        max_pos = np.max(data - mean)
        max_neg = np.abs(np.min(data - mean))
...
def auto_select_blobs(data, z_threshold=2.0, min_size=3):
    ...
    threshold = zscore_threshold(data, z_threshold, mode='both')
...
def main():
    ...
    # color scale - fixed range from -800 to 0
    global vmin
    global vmax
    ...
    # Calibration state
    calibration_enabled = [False]  # Track if calibration is enabled (RBF)
    compensation_map = [None]  # Store loaded compensation map (RBF)
    
    # JIT (Just-In-Time) calibration state
    jit_calibration_enabled = [False]  # Track if JIT calibration is enabled
    jit_compensation_map = [None]  # Store JIT compensation map
    jit_calibration_frame = [None]  # Frame index where JIT calibration was triggered
    jit_original_avg = [None]  # Average of frames after calibration frame (without compensation)

    def get_display_data(frame_idx):
        """Get data for display, applying calibration if enabled."""
        data = measured[frame_idx].copy()
        
        # Apply JIT calibration if enabled (highest priority)
        if jit_calibration_enabled[0] and jit_compensation_map[0] is not None:
            if jit_calibration_frame[0] is not None and frame_idx >= jit_calibration_frame[0]:
                if combined_mask[0] is not None:
                    # Apply JIT compensation only to selected pixels
                    data[combined_mask[0]] = data[combined_mask[0]] * jit_compensation_map[0][combined_mask[0]]
        
        # Apply RBF calibration if enabled
        elif calibration_enabled[0] and compensation_map[0] is not None:
            if combined_mask[0] is not None:
                # Apply compensation only to selected pixels
                data[combined_mask[0]] = data[combined_mask[0]] * compensation_map[0][combined_mask[0]]
    ...
    # Add thin black gridlines at pixel boundaries
    ...
    # Add tolerance display text above the heatmap (figure coordinates)
    tolerance_text = fig.text(0.78, 0.96, f'Tolerance: {tolerance[0]:.1f}',
                             fontsize=10,
                             verticalalignment='top',
                             horizontalalignment='right',
                             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

    # Add timer display above the heatmap (figure coordinates)
    start_time = time.time()
    timer_text = fig.text(0.24, 0.96, f'Time: 0:00',
                        fontsize=10,
                        verticalalignment='top',
    ...
    def update_frame(frame_idx):
        ...
        # Get display data (with calibration if enabled)
        display_data = get_display_data(frame_idx)
    ...
    def on_click(event):
        ...
        if event.button == 1:  # Left click - add to selection
            ...
            # Show calibration effect if enabled
            if calibration_enabled[0] and compensation_map[0] is not None:
                print(f"\n{'='*60}")
                print(f"CALIBRATION ACTIVE - Pixel values changed:")
                print(f"{'='*60}")
                
                # Show a few example pixels from the newly selected region
                num_examples = min(5, len(y_coords))
                for i in range(num_examples):
                    y, x = y_coords[i], x_coords[i]
                    original_val = current_data[y, x]
                    comp_factor = compensation_map[0][y, x]
                    calibrated_val = original_val * comp_factor
                    print(f"  Pixel ({y}, {x}): {original_val:.2f} × {comp_factor:.3f} = {calibrated_val:.2f}")
                
                if len(y_coords) > num_examples:
                    print(f"  ... and {len(y_coords) - num_examples} more pixels")
                print(f"{'='*60}\n")
    ...
    def on_hover(event):
        ...
        if event.inaxes == ax and event.xdata is not None and event.ydata is not None:
            ...
            # Use calibrated data if calibration is enabled, otherwise use original data
            current_data = get_display_data(current_frame[0])
    ...
    def update_overlay():
        ...
        # Update display data (apply calibration if enabled)
        display_data = get_display_data(current_frame[0])
        im.set_array(display_data)
        
        # Update cell value texts with calibrated values
        H, W = display_data.shape
        for y in range(H):
            for x in range(W):
                value = display_data[y, x]
    ...
    def print_statistics(event=None):
        ...
        # Note: Use original data for separation logic
        individual_regions = separate_regions(combined_mask[0], measured[current_frame[0]])
        ...
        if jit_calibration_enabled[0] and jit_compensation_map[0] is not None:
            print(f"CALIBRATION: JIT ENABLED (showing JIT calibrated values)")
        elif calibration_enabled[0] and compensation_map[0] is not None:
            print(f"CALIBRATION: RBF ENABLED (showing RBF calibrated values)")
        else:
            print(f"CALIBRATION: DISABLED (showing original values)")
        print(f"{'='*60}")
    ...
    def save_regions(event=None):
        ...
        # Determine frame range (current frame + N after, or as many as available)
        start_frame = current_frame[0]
        num_frames_to_capture = int(frames_to_save[0])  # Total frames to save starting from current
        end_frame = min(start_frame + num_frames_to_capture, T)
        num_frames = end_frame - start_frame
        frame_indices = list(range(start_frame, end_frame))

        print(f"\nSaving regions:")
        print(f"  Requested: {frames_to_save[0]} frames")
        ...
        # Create output filename based on input file and calibration status
        if jit_calibration_enabled[0]:
            output_file = regions_dir / (Path(current_file[0]).stem + "_regions_jit_calibrated.npz")
        elif calibration_enabled[0]:
            output_file = regions_dir / (Path(current_file[0]).stem + "_regions_calibrated.npz")
        else:
        ...
        region_data = {
            ...
            'calibration_enabled': calibration_enabled[0],
            'jit_calibration_enabled': jit_calibration_enabled[0]
        }
        
        # Save JIT compensation map info if enabled
        if jit_calibration_enabled[0] and jit_compensation_map[0] is not None:
            region_data['jit_compensation_map'] = jit_compensation_map[0]
            region_data['jit_calibration_frame'] = jit_calibration_frame[0]
            region_data['jit_compensation_map_applied'] = True
        
        # Save compensation map info if RBF calibration is enabled
        if calibration_enabled[0] and compensation_map[0] is not None:
            region_data['compensation_map_applied'] = True
            region_data['compensation_map_path'] = 'calibration/compensation_map_combined_all_sessions.npy'
        else:
            region_data['compensation_map_applied'] = False
        ...
        for idx, region_mask in enumerate(individual_regions):
            ...
            # Calculate centroid for cropping and save as relative location
            ...
            # Save relative location (centroid position in image coordinates)
            region_data[f'region_{idx}_relative_location'] = (cy, cx)
            region_data[f'region_{idx}_relative_location_mm'] = (
                cy * pixel_spacing_mm, 
                cx * pixel_spacing_mm
            )
            ...
            all_frames_crops = []
            all_frames_crops_calibrated = []  # For calibrated data
            ...
            for frame_idx in frame_indices:
                ...
                # If JIT calibration is enabled and we're at/after calibration frame, apply it
                if jit_calibration_enabled[0] and jit_compensation_map[0] is not None:
                    if jit_calibration_frame[0] is not None and frame_idx >= jit_calibration_frame[0]:
                        # Apply JIT calibration to the full frame data for selected pixels only
                        frame_data_calibrated = frame_data.copy()
                        frame_data_calibrated[region_mask] = frame_data[region_mask] * jit_compensation_map[0][region_mask]
                        
                        # Extract calibrated crop
                        crop_calibrated = frame_data_calibrated[y_min:y_max, x_min:x_max]
                        
                        # Pad if near boundary
                        if crop_calibrated.shape != (crop_size, crop_size):
                            padded_calibrated = np.zeros((crop_size, crop_size), dtype=crop_calibrated.dtype)
                            padded_calibrated[y_offset:y_offset+crop_calibrated.shape[0],
                                            x_offset:x_offset+crop_calibrated.shape[1]] = crop_calibrated
                            crop_calibrated = padded_calibrated
                        
                        all_frames_crops_calibrated.append(crop_calibrated)
                    else:
                        # Before calibration frame, save uncalibrated
                        all_frames_crops_calibrated.append(crop)

                # If RBF calibration is enabled, also save calibrated crop (only for selected pixels)
                elif calibration_enabled[0] and compensation_map[0] is not None:
                    # Apply compensation to the full frame data for selected pixels only
                    frame_data_calibrated = frame_data.copy()
                    frame_data_calibrated[region_mask] = frame_data[region_mask] * compensation_map[0][region_mask]
                ...
                # If any calibration is enabled, also save calibrated stats
                if (jit_calibration_enabled[0] and jit_compensation_map[0] is not None and frame_idx >= jit_calibration_frame[0]) or \
                   (calibration_enabled[0] and compensation_map[0] is not None):
                    # Use the calibrated frame data that was created above
                    if 'frame_data_calibrated' in locals():
                        stats_calibrated = calculate_region_stats(region_mask, frame_data_calibrated, pixel_spacing_mm)
                        region_data[f'region_{idx}_frame_{frame_idx}_stats_calibrated'] = stats_calibrated
            ...
            # Stack calibrated crops if any calibration is enabled
            if (jit_calibration_enabled[0] or calibration_enabled[0]) and len(all_frames_crops_calibrated) > 0:
                region_data[f'region_{idx}_crops_calibrated'] = np.stack(all_frames_crops_calibrated, axis=0)

            # Compute time-averaged centroids across all frames
            centroid_x_sum = 0.0
            centroid_y_sum = 0.0
            amplitude_sum = 0.0

            centroid_x_sum_calibrated = 0.0
            centroid_y_sum_calibrated = 0.0
            amplitude_sum_calibrated = 0.0

            for frame_idx in frame_indices:
                stats = region_data[f'region_{idx}_frame_{frame_idx}_stats']
                centroid_x_sum += stats['centroid_x']
                centroid_y_sum += stats['centroid_y']
                amplitude_sum += stats['min']

                if calibration_enabled[0]:
                    stats_calibrated = region_data[f'region_{idx}_frame_{frame_idx}_stats_calibrated']
                    centroid_x_sum_calibrated += stats_calibrated['centroid_x']
                    centroid_y_sum_calibrated += stats_calibrated['centroid_y']
                    amplitude_sum_calibrated += stats_calibrated['min']

            # Compute averages
            num_frames_actual = len(frame_indices)
            region_data[f'region_{idx}_centroid_x_avg'] = centroid_x_sum / num_frames_actual
            region_data[f'region_{idx}_centroid_y_avg'] = centroid_y_sum / num_frames_actual
            region_data[f'region_{idx}_amplitude_avg'] = amplitude_sum / num_frames_actual

            if calibration_enabled[0]:
                region_data[f'region_{idx}_centroid_x_avg_calibrated'] = centroid_x_sum_calibrated / num_frames_actual
                region_data[f'region_{idx}_centroid_y_avg_calibrated'] = centroid_y_sum_calibrated / num_frames_actual
                region_data[f'region_{idx}_amplitude_avg_calibrated'] = amplitude_sum_calibrated / num_frames_actual
        ...
        for idx, region_mask in enumerate(individual_regions):
            ...
            print(f"Relative location (centroid): y={region_data[f'region_{idx}_relative_location'][0]}, x={region_data[f'region_{idx}_relative_location'][1]}")
            print(f"Relative location (mm): y={region_data[f'region_{idx}_relative_location_mm'][0]:.1f}mm, x={region_data[f'region_{idx}_relative_location_mm'][1]:.1f}mm")
            
            # Print calibrated crop if available
            if calibration_enabled[0] and f'region_{idx}_crops_calibrated' in region_data:
                print(f"\n{crop_size}x{crop_size} Calibrated Crop (RBF, first frame):")
                crop_calibrated_first_frame = region_data[f'region_{idx}_crops_calibrated'][0]
                np.set_printoptions(precision=2, suppress=True, linewidth=200)
                print(crop_calibrated_first_frame)
                np.set_printoptions()  # Reset to defaults
        ...
        # Save centroids and amplitudes to text file
        centroid_file = output_file.with_suffix('.txt')
        with open(centroid_file, 'w') as f:
            # Write header based on calibration status
            if calibration_enabled[0]:
                f.write(f"# Region centroids and amplitudes (RBF calibrated)\n")
                f.write("# Time-averaged values across all frames\n")
                f.write("# Format: region_id, x_avg, y_avg, amplitude_avg, amplitude_calibrated_avg\n")
            else:
                f.write("# Region centroids and amplitudes\n")
                f.write("# Time-averaged values across all frames\n")
                f.write("# Format: region_id, x_avg, y_avg, amplitude_avg\n")
            f.write("#\n")

            for idx in range(num_regions):
                # Get time-averaged values
                cx_avg = region_data[f'region_{idx}_centroid_x_avg']
                cy_avg = region_data[f'region_{idx}_centroid_y_avg']
                amplitude_avg = region_data[f'region_{idx}_amplitude_avg']

                # If calibration is enabled, also write calibrated values
                if calibration_enabled[0]:
                    cx_avg_calibrated = region_data[f'region_{idx}_centroid_x_avg_calibrated']
                    cy_avg_calibrated = region_data[f'region_{idx}_centroid_y_avg_calibrated']
                    amplitude_avg_calibrated = region_data[f'region_{idx}_amplitude_avg_calibrated']
                    f.write(f"{idx}, {cx_avg}, {cy_avg}, {amplitude_avg:.4f}, {amplitude_avg_calibrated:.4f}\n")
                else:
                    f.write(f"{idx}, {cx_avg}, {cy_avg}, {amplitude_avg:.4f}\n")

        print(f"{'='*60}")
        print(f"Saved {num_regions} region(s) to: {output_file}")
        print(f"Saved region centroids to: {centroid_file}")
        print(f"Frames: {start_frame + 1} to {end_frame} ({num_frames} frames total)")
        print(f"Tolerance used: {tolerance[0]:.2f}")
        if calibration_enabled[0]:
            print(f"Calibration: RBF ENABLED (compensation map applied to selected pixels)")
            print(f"  Saved both original and calibrated crops")
        else:
    ...
    def toggle_calibration(event):
        """Toggle calibration on/off and load compensation map if needed."""
        if not calibration_enabled[0]:
            # Try to load compensation map
            compensation_map_path = Path('calibration/compensation_map_combined_all_sessions.npy')
            if not compensation_map_path.exists():
                print(f"\nError: Compensation map not found at {compensation_map_path}")
                print("Please generate a compensation map first using calibration.py")
                return
            
            try:
                compensation_map[0] = np.load(compensation_map_path)
                print(f"\n{'='*70}")
                print(f"Loaded compensation map from: {compensation_map_path}")
                print(f"Map shape: {compensation_map[0].shape}")
                print(f"Map range: {compensation_map[0].min():.3f} to {compensation_map[0].max():.3f}")
                print(f"{'='*70}\n")
                
                # Verify dimensions match
                H, W = measured.shape[1], measured.shape[2]
                if compensation_map[0].shape != (H, W):
                    print(f"Warning: Compensation map shape {compensation_map[0].shape} doesn't match data shape ({H}, {W})")
                    compensation_map[0] = None
                    return
                
                calibration_enabled[0] = True
                btn_calibrate.label.set_text('Calibrate: ON')
                btn_calibrate.color = 'lightgreen'
                
                # Show debug output if regions are selected
                if combined_mask[0] is not None:
                    current_data = measured[current_frame[0]]
                    y_coords, x_coords = np.where(combined_mask[0])
                    
                    print(f"\n{'='*70}")
                    print(f"CALIBRATION ENABLED - SHOWING CHANGES")
                    print(f"{'='*70}")
                    print(f"Selected pixels: {len(y_coords)}")
                    ...
                    print("No regions selected yet. Select regions to see calibration effects.")
                
                print("Calibration enabled - selected pixels will be compensated in display and saved data")
                
                # Refresh the display
                update_frame(current_frame[0])
                
            except Exception as e:
                print(f"Error loading compensation map: {e}")
                return
        else:
            # Disable calibration
            calibration_enabled[0] = False
            btn_calibrate.label.set_text('Calibrate: OFF')
            btn_calibrate.color = 'lightgray'
    ...
    btn_calibrate_ax = fig.add_axes([0.08, 0.16, 0.10, 0.04])
    btn_calibrate = Button(btn_calibrate_ax, 'Calibrate: OFF', color='lightgray')
    btn_calibrate.on_clicked(toggle_calibration)
    
    # Create JIT calibrate button  
    btn_jit_calibrate_ax = fig.add_axes([0.20, 0.16, 0.10, 0.04])
    btn_jit_calibrate = Button(btn_jit_calibrate_ax, 'JIT Cal: OFF', color='lightgray')
    
    def toggle_jit_calibration(event):
        """Toggle JIT (Just-In-Time) calibration on/off."""
        if not jit_calibration_enabled[0]:
            # Enable JIT calibration
            if combined_mask[0] is None:
                print("\nError: No regions selected. Please select regions first.")
                return
            
            current_idx = current_frame[0]
            print(f"\n{'='*70}")
            print(f"JIT CALIBRATION ENABLED at frame {current_idx}")
            print(f"{'='*70}")
            
            # Get current frame data
            current_data = measured[current_idx].copy()
            
            # Goal: reduce INTER-region variability while preserving INTRA-region variability
            # For each region, compute its average
            # Then apply a uniform multiplicative factor to the entire region to normalize all regions to the same average
            
            if len(region_stack[0]) == 0:
                print("Error: No regions defined")
                return
            
            # Compute average for each region
            region_averages = []
            for region_mask in region_stack[0]:
                region_avg = np.mean(current_data[region_mask])
                region_averages.append(region_avg)
            
            # Compute global average across all regions
            global_avg = np.mean(region_averages)
            
            print(f"  Number of regions: {len(region_stack[0])}")
            print(f"  Individual region averages: {[f'{avg:.2f}' for avg in region_averages]}")
            print(f"  Global average (target): {global_avg:.2f}")
            
            # Create compensation map: for each region, apply uniform factor = global_avg / region_avg
            # This brings each region's average to the global average while preserving spatial patterns
            H, W = current_data.shape
            jit_compensation_map[0] = np.ones((H, W), dtype=np.float32)
            
            for region_idx, region_mask in enumerate(region_stack[0]):
                region_avg = region_averages[region_idx]
                if abs(region_avg) > 1e-6:  # Avoid division by very small numbers
                    compensation_factor = global_avg / region_avg
                else:
                    compensation_factor = 1.0
                
                # Apply the SAME compensation factor to ALL pixels in this region
                jit_compensation_map[0][region_mask] = compensation_factor
                print(f"  Region {region_idx}: avg={region_avg:.2f}, factor={compensation_factor:.3f}")
            
            # Compute average of frames after this frame (without compensation)
            if current_idx < T - 1:
                # Average all subsequent frames
                subsequent_frames = measured[current_idx+1:, :, :]
                avg_subsequent = np.mean(subsequent_frames, axis=0)
                jit_original_avg[0] = avg_subsequent.copy()
                
                # Compute per-region averages before compensation
                original_region_avgs = []
                original_region_stds = []
                for region_mask in region_stack[0]:
                    region_values = avg_subsequent[region_mask]
                    original_region_avgs.append(np.mean(region_values))
                    original_region_stds.append(np.std(region_values))
                
                # Inter-region variability (variability between region averages)
                inter_region_std_original = np.std(original_region_avgs)
                # Average intra-region variability (average of within-region stds)
                avg_intra_region_std_original = np.mean(original_region_stds)
                
                # Apply compensation and recompute
                compensated_region_avgs = []
                compensated_region_stds = []
                for region_mask in region_stack[0]:
                    region_values = avg_subsequent[region_mask]
                    compensated_values = region_values * jit_compensation_map[0][region_mask]
                    compensated_region_avgs.append(np.mean(compensated_values))
                    compensated_region_stds.append(np.std(compensated_values))
                
                inter_region_std_compensated = np.std(compensated_region_avgs)
                avg_intra_region_std_compensated = np.mean(compensated_region_stds)
                
                print(f"\n  Variability Analysis (subsequent frames average):")
                print(f"    INTER-region variability (between regions):")
                print(f"      Without compensation: std={inter_region_std_original:.2f}")
                print(f"      With compensation:    std={inter_region_std_compensated:.2f}")
                
                print(f"    INTRA-region variability (within regions, averaged):")
                print(f"      Without compensation: avg_std={avg_intra_region_std_original:.2f}")
                print(f"      With compensation:    avg_std={avg_intra_region_std_compensated:.2f}")
                
                if inter_region_std_compensated < inter_region_std_original:
                    improvement_pct = ((inter_region_std_original - inter_region_std_compensated) / inter_region_std_original) * 100
                    print(f"  ✓ INTER-region variability REDUCED by {improvement_pct:.1f}%")
                else:
                    print(f"  ⚠ Warning: INTER-region variability increased")
                
                # Check that intra-region variability is preserved (should be similar)
                intra_change_pct = abs(avg_intra_region_std_compensated - avg_intra_region_std_original) / avg_intra_region_std_original * 100
                if intra_change_pct < 5:  # Less than 5% change
                    print(f"  ✓ INTRA-region variability PRESERVED (changed by {intra_change_pct:.1f}%)")
                else:
                    print(f"  ⚠ INTRA-region variability changed by {intra_change_pct:.1f}%")
            else:
                print(f"  Note: At last frame, cannot analyze subsequent frames")
            
            jit_calibration_enabled[0] = True
            jit_calibration_frame[0] = current_idx
            btn_jit_calibrate.label.set_text('JIT Cal: ON')
            btn_jit_calibrate.color = 'yellow'
            ...
            # Disable JIT calibration
            jit_calibration_enabled[0] = False
            jit_compensation_map[0] = None
            jit_calibration_frame[0] = None
            jit_original_avg[0] = None
            btn_jit_calibrate.label.set_text('JIT Cal: OFF')
            btn_jit_calibrate.color = 'lightgray'
            print("JIT Calibration disabled")
    ...
    btn_jit_calibrate.on_clicked(toggle_jit_calibration)
    ...
    def toggle_labels(label):
        ...
        # Update cell values with calibrated data if calibration is enabled
        if show_labels[0]:
            display_data = get_display_data(current_frame[0])
            for y in range(len(cell_value_texts)):
                for x in range(len(cell_value_texts[y])):
                    ...
                    color = 'green' if value > 0 else 'red'
                    cell_value_texts[y][x].set_color(color)
                    cell_value_texts[y][x].set_visible(True)
        else:
            for y in range(len(cell_value_texts)):
                for x in range(len(cell_value_texts[y])):
                    cell_value_texts[y][x].set_visible(False)
    ...
    slider_zscore_ax = fig.add_axes([0.20, 0.06, 0.40, 0.02])
    ...
    slider_patchsize_ax = fig.add_axes([0.20, 0.03, 0.40, 0.02])
    ...
    def load_file_by_index(file_idx):
        ...
        auto_detect_zscore(None)  # Auto-detect regions on new file load
        fig.canvas.draw_idle()
    ...
    auto_detect_zscore(None)  # Initial auto-detection on load
